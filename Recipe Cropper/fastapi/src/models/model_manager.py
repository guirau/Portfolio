...
class ModelManager:
    ...
    def check_model_files(self, model_type: str) -> None:
        """
        ...
        if model_type == "segformer":
            if Path(SEGFORMER_MODEL_PATH).exists():
                logger.info("NVIDIA/SegFormer model files already exist locally")
            else:
                logger.info("Downloading NVIDIA/SegFormer model files")

                # Create directory if it doesn't exist
                local_path = Path(SEGFORMER_MODEL_PATH).parent
                Path.mkdir(local_path, parents=True, exist_ok=True)

                # Download model files
                download_path = f"{self.models_s3_path}/segformer"
                self._download_model_files(download_path, local_path)

        elif model_type == "briaai":
            if Path(BRIAAI_BASE_MODEL_PATH).exists():
                logger.info("BRIAAI/RMBG-2.0 model files already exist locally")
            else:
                logger.info("Downloading BRIAAI/RMBG-2.0 model files")

                # Create directory if it doesn't exist
                local_path = Path(BRIAAI_BASE_MODEL_PATH).parent
                Path.mkdir(local_path, parents=True, exist_ok=True)

                # Download model files
                download_path = f"{self.models_s3_path}/briaai"
                self._download_model_files(download_path, local_path)
